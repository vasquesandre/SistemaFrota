...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['index'])) {
    ...
    if ($search_id_result->num_rows > 0) {
        ...
        $sql = "UPDATE tb_veiculo SET disponivel = 1 WHERE id_veiculo = ?";
        ...
        if ($stmt_veiculo->affected_rows > 0) {
            ...
            if ($result_search->num_rows > 0) {
                $sql_login_veiculo = "UPDATE tb_login_veiculo SET disponivel = 1 WHERE id_veiculo = ?";
                ...
                if ($stmt_login_veiculo->affected_rows > 0) {
                    echo json_encode(array("status" => "success", "message" => "Veículo reativado com sucesso."));
                } else {
                    echo json_encode(array("status" => "error", "message" => "Não foi possível reativar o veículo, tente novamente."));
                }
            } else {
                echo json_encode(array("status" => "success", "message" => "Veículo reativado com sucesso."));
            }
